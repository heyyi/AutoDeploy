...
NodeA_HostName="rhel1"
NodeB_HostName="rhel2"
INTERFACE="eth0"
FENCE_DEV1="/dev/sdb"
DEV1="/dev/sdc"
DEV2="/dev/sdd"
...
YumSever="10.108.104.189"
NTPServer="10.106.16.22"
#=============================================================================================

LocalNode=`hostname`
LocalIP=` ip addr show dev eth0  |grep inet | grep -v "inet6" |awk '{print $2}' | awk -F / '{print $1}'`
...
then
   ...
   if [ ! -b $FENCE_DEV1 ]
  then
       echo "block device $FENCE_DEV1 doesn't exist"
          exit 1
  fi
...
pcs status
pvcreate $DEV1 $DEV2
vgcreate -cy -Ay cluster_vg $DEV1 $DEV2
#lvcreate -ncluster_lv -l90%VG cluster_vg --config 'global {locking_type = 0}'
lvcreate -W n -ncluster_lv -l100%VG cluster_vg
echo "mkfs.gfs2 /dev/cluster_vg/cluster_lv..."
mkfs.gfs2 -O -j2 -p lock_dlm -t RHCS:gfs2 /dev/cluster_vg/cluster_lv

echo "creating gfs resource..."
pcs resource create clusterfs Filesystem device="/dev/cluster_vg/cluster_lv" directory="/GFS2" fstype="gfs2" "options=noatime" op monitor interval=10s on-fail=fence clone interleave=true
...
{
echo "removing GFS2"
pcs resource delete clusterfs

vgchange -cn  cluster_vg --config 'global {locking_type = 0}'
lvremove /dev/cluster_vg/cluster_lv -f
vgremove cluster_vg
pvremove $DEV1 $DEV2
...
#       ssh $RemoteNode "systemctl restart nfs"
    ...
    ssh $RemoteNode "systemctl restart rpcbind.service"
        pcs resource create nfs_vg LVM volgrpname=nfs_vg exclusive=true --group nfsgroup
